class Source
  ...
  def self.search_records(pdf_records, txt_record)
    ...
    until index == txt_record.length
      address_index = pdf_records.index{|record| record.include?(txt_record[index][1])} if pdf_records.index{|record| record.include?(txt_record[index][1])}
      recipient_index = address_index - 1
      city_state_zip_index = recipient_index + 2
      recipient = pdf_records[recipient_index]
      if pdf_records[address_index].match?(/^(?:[Pp][Oo]\s[Bb][Oo][Xx]|[0-9]{1,}|[0-9]{1,}[A-Za-z]|[0-9]{1,}-[0-9]{1,})\s(?:[0-9A-Za-z\.'#]|[^\S\r\n])+/)
